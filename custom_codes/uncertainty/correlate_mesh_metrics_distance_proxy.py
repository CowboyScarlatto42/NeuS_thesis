...
def proxy_values_from_hessian(hessian, eps):
    return {
        'hessian': hessian,
...
def write_csv(path, points, distances, hessian, proxies):
    fieldnames = [
        'x_norm',
        'y_norm',
        'z_norm',
    ...
    with open(path, 'w', newline='') as f:
        ...
        for idx in range(points.shape[0]):
            writer.writerow({
                ...
                'hessian': hessian[idx],
                'negative_hessian': proxies['negative_hessian'][idx],
                'log10_hessian': proxies['log10_hessian'][idx],
                'inverse_log10_hessian': proxies['inverse_log10_hessian'][idx],
            })
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            ...
            'a Hessian/uncertainty proxy on the same normalized reconstruction samples.'
        )
    )
    ...
    proxies = proxy_values_from_hessian(hessian, args.eps)
