...
def load_colmap_data(realdir):
    ...
    # Map COLMAP image_id -> 0..N-1 index (iteration order used for pose arrays)
    id_to_idx = {k: i for i, k in enumerate(imdata.keys())}
    ...
    poses = np.concatenate(
        [poses[:, 1:2, :], poses[:, 0:1, :], -poses[:, 2:3, :], poses[:, 3:4, :], poses[:, 4:5, :]], 1
    )

    return poses, pts3d, perm, id_to_idx


def save_poses(basedir, poses, pts3d, perm, id_to_idx):
    pts_arr = []
    vis_arr = []
    for k in pts3d:
        ...
        for image_id in pts3d[k].image_ids:
            if image_id not in id_to_idx:
                # track might reference an image outside current registered set
                continue
            cams[id_to_idx[image_id]] = 1
    ...
    for r in factors + resolutions:
        ...
        if isinstance(r, int):
            ...
            imgs_down = skimage.transform.resize(
                imgs, [imgs.shape[0], r[0], r[1], imgs.shape[-1]],
...
def gen_poses(basedir, match_type, factors=None):
    ...
    poses, pts3d, perm, id_to_idx = load_colmap_data(basedir)
    save_poses(basedir, poses, pts3d, perm, id_to_idx)
